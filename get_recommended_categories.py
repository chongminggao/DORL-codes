...
def test_whether_dominate(xy_predict, df_true_list, res_train, res_test, df_feat, df_train_sorted_index, epoch,
                          top_visual, message):
    # top_visual = 0.5
...
def setup_world_model(args, x_columns, y_columns, ab_columns, task, task_logit_dim, is_ranking, MODEL_SAVE_PATH):
    ...
    myfunc = functools.partial(test_whether_dominate, res_train=res_train, res_test=res_test, df_feat=df_feat,
                               df_train_sorted_index=df_train_sorted_index, top_visual=args.top_visual, message=args.message)
