...
from environments.KuaiRand_Pure.env.KuaiRand import KuaiRandEnv, negative_sampling, construct_complete_val_x, \
    compute_normed_reward_for_all
...
def load_dataset_kuairand(user_features, item_features, reward_features, tau, entity_dim, feature_dim, MODEL_SAVE_PATH):
    df_train, df_user, df_feat, list_feat = KuaiRandEnv.get_df_kuairand("log_standard_4_08_to_4_21_pure.csv")
...
def load_static_validate_data_kuairand(user_features, item_features, reward_features, entity_dim, feature_dim,
                                       DATAPATH):
    df_val, df_user, df_feat, list_feat = KuaiRandEnv.get_df_kuairand("log_random_4_22_to_5_08_pure.csv")
    ...
    dataset_val.set_df_item_val(df_item_env)
